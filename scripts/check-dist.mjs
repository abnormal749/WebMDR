...
// The owner's deployment may name one counter endpoint (see vite.config.ts); nothing else external.
const counter = process.env.WEBMDR_GOATCOUNTER ? new URL(process.env.WEBMDR_GOATCOUNTER) : undefined;
if (counter) {
  check(html.includes(`<meta name="webmdr-counter" content="${counter.href}"`), 'counter meta tag missing');
  ...
  check(!html.includes('webmdr-counter'), 'counter present without WEBMDR_GOATCOUNTER');
}
