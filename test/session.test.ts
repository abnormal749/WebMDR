...
describe('transactions', () => {
  it('sends the exact GET bytes and completes on ACK then reply', async () => {
    const session = open();
    const result = session.request(getNoiseOperation(XM5));
  ...
  it('completes on reply before ACK, and records the late ACK on the same receipt', async () => {
    const session = open();
    const p = session.request(getNoiseOperation(XM5));
  ...
  it('accepts a reply delivered synchronously during the write', async () => {
    ...
    await expect(session.request(getNoiseOperation(XM5))).resolves.toMatchObject({ value: AMBIENT_12 });
  });
  ...
  it('never uses a reply received before the request as a fresh result', async () => {
    ...
    await expectError(session.request(getNoiseOperation(XM5)), 'desynchronized', 'not-sent');
  ...
  it('rejects out-of-range and unknown enum values', async () => {
    const session = open();
    let result = session.request(getNoiseOperation(XM5));
    channel.deliver(data(1, '67 17 01 01 01 00 15'));
    await expectError(result, 'malformed', 'replied');
    result = session.request(getNoiseOperation(XM5));
  ...
  it('allows one transaction at a time without queueing', async () => {
    const session = open();
    const first = session.request(getNoiseOperation(XM5));
    await expectError(session.request(getNoiseOperation(XM5)), 'busy', 'not-sent');
  ...
  it('alternates the TX sequence after each completed transaction', async () => {
    ...
    channel.deliver(ack(1), fixtures.noiseReplyAmbient12);
    await first;
    const second = session.request(getNoiseOperation(XM5));
...
describe('duplicates and ACKs', () => {
  ...
  it('completes a setter only on the ACK the audited dialect implies', async () => {
    ...
    const result = session.command(setNoiseOperation(XM5, AMBIENT_12));
...
describe('modes', () => {
  it('read-only refuses setting changes without writing', async () => {
    ...
    expect(() => setNoiseOperation(XM5, AMBIENT_12)).not.toThrow();
    await expectError(session.command(setNoiseOperation(XM5, AMBIENT_12)), 'mode', 'not-sent');
    expect(channel.written).toEqual([]);
  });

  it('passive mode sends nothing, not even ACKs', async () => {
    const session = open('passive');
    await expectError(session.request(getNoiseOperation(XM5)), 'mode', 'not-sent');
...
describe('writer scheduling', () => {
  it('writes ACKs ahead of queued data, one complete frame at a time', async () => {
    ...
    await flush();
    const result = session.request(getNoiseOperation(XM5)); // queued behind the blocked ACK
  ...
  it('does not match a reply that arrives while the request is still queued, not yet written', async () => {
    ...
    await flush();
    const result = session.request(getNoiseOperation(XM5)).then((r) => r.value);
...
describe('lifecycle', () => {
  it('closes during a pending read and pending request', async () => {
    ...
    await expectError(session.request(getNoiseOperation(XM5)), 'closed', 'not-sent');
  });
  ...
  it('ignores a write completing after the session closed', async () => {
    ...
    const g = gate();
    channel.writeGate = g.promise;
    const result = session.request(getNoiseOperation(XM5)).catch((e: unknown) => e);
    await flush();
    await session.close();
    g.open();
    await flush();
